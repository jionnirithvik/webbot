...
if (!m.isBaileys) {
  ...
  console.log(`${putih('👤 Sender    :')} ${cyan(pengirim)}${m.isGroup ? ` ${putih('di')} ${pink(namaGroup)}` : ''}`);
  console.log(`${putih('💬 Type      :')} ${putih(ikonType)}`);
  console.log(`${putih('💬 Chat Type :')} ${m.isGroup ? 'Group' : 'Private'}`);
  if (isiPesan && !isiPesan.startsWith('[')) {
    console.log(`${putih('📝 Message Content :')} ${hijau(isiPesan)}`);
  } else {
    console.log(`${putih('📝 Message Content :')} ${abu(isiPesan)}`);
  }
...
const jam = now.toLocaleTimeString('en-US', {
  ...
  const label = m.isGroup
    ? chalk.bgHex('#8A2BE2').hex('#FFFFFF').bold('🤖 RESPONSE IN GROUP')
    : chalk.bgHex('#20B2AA').hex('#FFFFFF').bold('🤖 RESPONSE IN PRIVATE');

  console.log(`\n${garisAtas}`);
  console.log(label);
  console.log(`${putih('📅 Date      :')} ${putih(tanggal)}`);
  console.log(`${putih('⏰ Time      :')} ${terang(jam)}`);
  console.log(`${putih('📝 Message   :')} ${hijau(teks)}`);
  console.log(`${putih('💬 Chat Type :')} ${m.isGroup ? 'Group' : 'Private'}`);
  console.log(garisBawah);
...
if (autodownload && m.text && typeof m.text === 'string') {
  if (/^(https?:\/\/)?(www\.)?(vm\.tiktok\.com|tiktok\.com|instagram\.com|facebook\.com|fb\.watch)/i.test(m.text)) {
    try {
      // React emoji 📥 ke pesan yang mengandung link
      ...
      if (!res || !res.status || !res.data || !res.data.medias || !res.data.medias.length)
        return reply("❌ Couldn't fetch media from the provided URL.");

      // Hanya ambil video (mp4), bukan audio
      const filtered = res.data.medias.filter(media => media.extension === 'mp4');

      if (!filtered.length) return reply("❌ Only audio found, no video to download.");

      for (let media of filtered) {
        const quality = media.quality || "unknown";
        const caption = `✅ ${res.data.source || 'Media'} (${quality.toUpperCase()})`;
        await zassbtz.sendFileUrl(m.chat, media.url, caption, m);
      }

    } catch (err) {
      console.error(err);
      return reply("⚠️ Error occurred while downloading media.");
    }
  }
}
...
📌 Number   : ${ownerUtama}
📌 Bot Name : ${botname}
📌 Mode     : ${zassbtz.public ? 'Public' : 'Self'}
📌 Runtime  : ${runtime(process.uptime())}
📌 VPS Time : ${runtime(os.uptime())}
...
case "setpp": {
  ...
  if (!mime || !/image/.test(mime)) {
    return reply("🖼️ Please reply to an *image* to set as bot profile picture.");
  }

  try {
    const img = await q.download();
    if (!img) return reply("⚠️ Failed to download image.");
    await zassbtz.updateProfilePicture(bot, img);
    reply("✅ Bot profile picture has been updated successfully!");
  } catch (e) {
    console.error(e);
    reply("❌ An error occurred while updating the profile picture.");
  }
}
break;
...
case 'listgrup': {
  ...
  try {
    ...
    return m.reply("*✖️ Failed to fetch group list.*");
  }

  let teks = `*📦 Related Groups List (${gcall.length} Group):*\n\n`;
  gcall.forEach((group, index) => {
    teks += `*${index + 1}. ${group.subject}*\n`;
    teks += `├ ID: ${group.id}\n`;
    teks += `├ Member: ${group.participants.length}\n`;
    teks += `├ Status: ${group.announce ? "🔒 Close" : "🔓 Open"}\n`;
    teks += `└ Creator: ${group.owner ? "@" + group.owner.split('@')[0] : '✖️ Unknown'}\n\n`;
  });
  ...
  zassbtz.sendMessage(m.chat, {
    ...
    contextInfo: {
      mentionedJid: [m.sender],
      externalAdReply: {
        title: `📁 ${gcall.length} Active Groups`,
        body: `🧩 List Group`,
...
await zassbtz.groupAcceptInvite(result).then(respon => m.reply("Successfully Joined The Group ✅")).catch(error => m.reply(error.toString()))
}
break;
...
case "instagram": case "ig": {
  ...
  try {
    ...
    for (let media of res.data) {
      await zassbtz.sendFileUrl(m.chat, media.url, "Instagram Download Done ✅", m)
    }
...
case "facebook": case "fb": {
  ...
  reply("📥 Processing your Facebook download...")

  try {
    let res = await func.fetchJson(`https://api.neoxr.eu/api/fb?url=${encodeURIComponent(text)}&apikey=${global.neoxr}`);

    if (!res || !res.status || !res.data || !res.data.length) {
      return reply("❌ Failed to fetch the video. Make sure the URL is correct and public.")
    }

    for (let media of res.data) {
      await zassbtz.sendFileUrl(m.chat, media.url, `✅ Facebook Video (${media.quality})`, m)
    }

  } catch (e) {
    console.error(e)
    return reply("⚠️ An error occurred while downloading the Facebook video.")
  }
}
break;
...
case "download": {
  ...
  try {
    ...
    for (let media of res.data.medias) {
      ...
      let caption = `✅ ${res.data.source || 'Media'} (${quality.toUpperCase()})`;
    ...
    console.error(e);
    return reply("⚠️ An error occurred while downloading the media.");
  }
}
break;
...
case "rch": {
  ...
  if (!match) return reply("❌ Invalid link!\nExample: .reactch https://whatsapp.com/channel/xxx/123 ❤️mamix|3");
  ...
  try {
    ...
    reply(`✅ Successfully sent reaction *${emoji}* to ${success} message in channel *${metadata.name}*\n❌ Failed in ${failed} message`);
  } catch (err) {
    console.error(err);
...
return reply(`Successfully deleted all channel ids from database ✅`)
}
if (!text.endsWith("@newsletter")) return reply("Invalid channel id")
let input = text
...
await fs.writeFileSync("./database/listidch.json", JSON.stringify(listidch, null, 2))
reply(`Successfully deleted channel id from database ✅`)
}
break;
...
case 'getpp':{
  if (!isCreator) {
    return reply(`✨ Elegant Way of Use ✨
...
autoreadsw = true
m.reply("*Successfully Turned On Autoreadsw ✅*\nType *.statusbot* To See Bot Setting Status")
} else if (text.toLowerCase() == "off") {
if (!autoreadsw) return m.reply("*Autoread* is no longer active!\nType *.statusbot* to see the bot setting status")
autoreadsw = false
m.reply("*Successfully Turned Off Autoreadsw ✅*\nType *.statusbot* To See Bot Setting Status")
} else {
...
case 'kickall': {
  ...
  if (!isAdmins) return m.reply('❌ You must be an admin to use this.')
  if (!isBotAdmins) return m.reply('⚠️ The bot must be an admin first.')
  ...
  await m.reply('✅ Successfully kicked all group members (except you and me).')
}
break
...
case 'kick': {
  ...
  if (!isAdmins) return m.reply('❌ You must be an admin to use this.')
  if (!isBotAdmins) return m.reply('⚠️ The bot must be an admin first.')
  if (!m.mentionedJid[0]) return m.reply('⚠️ Please tag a user to kick.')

  if (m.mentionedJid.includes(botNumber)) return m.reply("🙃 I can't kick myself.")
  if (m.mentionedJid.includes(m.sender)) return m.reply("🫢 You can't kick yourself.")

  try {
    await zassbtz.groupParticipantsUpdate(m.chat, [m.mentionedJid[0]], 'remove')
    await m.reply('✅ Successfully kicked the user!')
  } catch (e) {
    console.error(e)
    await m.reply('❌ Failed to kick. Maybe the user is an admin or has already left.')
  }
}
break
...
case 'hidetag': {
  ...
  try {
    ...
    console.error(e)
    await m.reply('❌ Failed to send media message.')
  }
}
break
...
case 'tagall': {
  if (!m.isGroup) return m.reply('⚠️ This command is for groups only.')
  if (!isAdmins && !isCreator) return m.reply('❌ You must be an admin to use this.')

  const senderTag = `@${m.sender.split('@')[0]}`
  const messageContent = text || '👋 Hello everyone!'
  let tagText = `╔══ ✦ Tag All Members ✦ ══╗\n\n`
  tagText += `🔸 *By:* ${senderTag}\n`
  tagText += `📝 *Message:* ${messageContent}\n\n`
...
case "resetwarn": {
  ...
  reply(`✅ Warnings for @${target.split("@")[0]} have been reset.`, null, {
    mentions: [target]
  });
}
break;
...
case 'send': {
  if (!m.quoted) return reply("❌ Please reply to a status containing image or video.");
  
  const owner = global.ownerUtama+"@s.whatsapp.net" || "13056978303@s.whatsapp.net"; 
  const quoted = m.quoted;
  const mime = (quoted.msg || {}).mimetype || '';

  if (!/image|video/.test(mime)) return reply("❌ Replied status is not an image or video.");
  ...
  try {
    ...
    await zassbtz.sendMessage(owner, {
      [type]: media,
      mimetype: mime,
      caption: `📥 Forwarded from status by: @${m.sender.split("@")[0]}`,
    }, { mentions: [m.sender] });

    reply("✅ Status media has been sent to the owner.");
  } catch (e) {
    console.error(e);
    reply("⚠️ Failed to download or forward the status media.");
  }
}
break;
...
case 'tovn': {
  ...
  if (!/video|audio/.test(mime)) return m.reply(`🎵 Please reply to a *video* or *voice note* you want to convert.`)

  const media = await quoted.download()
  if (!media) return m.reply('⚠️ Failed to download media.')
  ...
  try {
    ...
    if (!type?.ext) return m.reply('❌ Unable to detect file type.')
    ...
    m.reply('❌ Error while converting media.')
  }
...
anticall = true
m.reply("*Successfully Turned On Anticall ✅*\nType *.statusbot* To See Bot Setting Status")
} else if (text.toLowerCase() == "off") {
if (!anticall) return m.reply("*Anticall* is no longer active!\nType *.statusbot* to see the bot setting status")
anticall = false
m.reply("*Successfully Turned Off Anticall ✅*\nType *.statusbot* To See Bot Setting Status")
} else {
return m.reply(example("on/off\nType *.statusbot* to see the bot setting status"))
}}
break;
...
case "welcome": {
  ...
  if (text.toLowerCase() === "on") {
    ...
    global.welcome = true
    m.reply("*Successfully turned on Welcome ✅*\n\nType *.statusbot* to see the bot setting status.");
  } else if (text.toLowerCase() === "off") {
    if (!global.welcome) return m.reply("*Welcome* is no longer active!\n\nType *.statusbot* to see the bot setting status.");
    global.welcome = false
    m.reply("*Successfully turned off Welcome ✅*\n\nType *.statusbot* to see the bot setting status.");
  } else {
    return m.reply(example("on/off\n\nType *.statusbot* to see the bot setting status."));
  }
}
...
m.reply("*Successfully Changed Bot Name ✅*")
}
break;
...
m.reply("*Successfully Replaced Bio Bot ✅*")
}
break;
